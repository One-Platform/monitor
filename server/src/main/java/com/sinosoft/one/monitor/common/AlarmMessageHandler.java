...
public class AlarmMessageHandler {
	...
	public void doMessage(MessageBase messageBase, String alarmId) {
		...
		for(int i=0, len=alarmMessageList.size(); i<len; i++) {
			...
			if(alarmAttribute.isAvailabilityAlarm()) {
				...
				if(StringUtils.equals(AvailabilityStatus.ERROR.value(),alarmAttribute.getAttributeValue().toString())) {
		...
		doAlarm(thresholdAlarmParams,messageBase);
	}
	 ...
	 */
	private void doAlarm(ThresholdAlarmParams thresholdAlarmParams,MessageBase messageBase) {
		...
		if(allAttributeActions != null && allAttributeActions.size() > 0) {
            //获取alarmId对应的
            //修改日期2013.12.25 alarmMessageBuilder 代表根本原因，在其尾部追加一行，将链接附上
            if (messageBase.hasLink()){
                alarmMessageBuilder.append("<br>");
                alarmMessageBuilder.append("<a href=\"");
                alarmMessageBuilder.append(messageBase.getLink());
                alarmMessageBuilder.append("\">");
                alarmMessageBuilder.append("点击查看详细");
                alarmMessageBuilder.append("</a>");
            }
