...
public class ExceptionAgentMessageService implements AgentMessageService{
    ...
    private LoadingCache<String ,Url> urlCache= CacheBuilder.newBuilder().build(new CacheLoader<String,Url>() {
        @Override
        public Url load(String key) throws Exception {
            return  urlRepository.findByUrl(key);
        }
    });
    /**
     * 第三版的处理方法
    ...
    public void handleMessage(String ip,String port,Map<String, Application> applicationMapByName, List<ExceptionInfo> exceptionInfoList) {
        ...
        for(ExceptionInfo exceptionInfo:exceptionInfoList){
            ...
            //告警
            MessageBase messageBase = alarmMessageBuilder.newMessageBase(applicationId)
            ...
            //如果是后台消息则不需要创建链接
            if (!StringUtils.equals(applicationId,ip+":"+port)){
                String urlId = null;
                if (url!=null){
                    try {
                        Url urlObj = urlCache.get(url);
                        urlId = urlObj.getId();
                    } catch (ExecutionException e) {
                        continue;
                    }
                }
                messageBase.subResourceId(urlId);
                messageBase.setLink(MessageBase.LinkTemplate.EXCEPTION)
                        .setLinkIds(applicationId, exceptionInfo.getUrlTraceId(), urlId);
            }
            messageBase.alarm();
        }
