...
public interface AlarmRepository extends PagingAndSortingRepository<Alarm, String> {
    ...
    @SQL("select severity from ge_monitor_alarm where monitor_id = ?1 and create_time between ?2 and ?3  order by create_time desc")
    Page<Alarm> findAlarmSeverityByMonitorId(String monitorId, Date startTime, Date endTime,Pageable pageable);
    ...
    @SQL("select severity from ge_monitor_alarm where monitor_id = ?1 and sub_resource_type=?2 " +
            "and sub_resource_id=?3 and create_time between ?4 and ?5 order by create_time desc")
    Page<Alarm> findAlarmPageByMonitorId(String monitorId, String subResourceType, String subResourceId, Date startTime, Date endTime,Pageable pageable);
	...
	@SQL("select to_number(to_char(create_time, 'HH24')) as time_index,  severity, count(1) as count" +
			"  from ge_monitor_alarm" +
			"  where monitor_id = ?1 and sub_resource_type=?2 and sub_resource_id=?3 and create_time between ?4 and ?5" +
			" group by to_char(create_time, 'yyyy-MM-dd HH24'), to_char(create_time, 'HH24'), severity")
