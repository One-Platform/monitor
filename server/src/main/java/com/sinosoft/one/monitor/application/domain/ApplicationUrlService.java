...
//				urlId, interval);
		...
		// 获得可用性信息
		UrlAvailableInf urlAvailableInf = applicationEmuService.getUrlAvailableToday(applicationId,urlId);

        //如果不可用则不健康
        if (urlAvailableInf.getTrend() == Trend.DROP){
		...
		applicationUrlInfoViewModel.setLatestFailTime(urlAvailableInf.getLatestTime() == null ? "无" :
	...
	public Page<MethodResponseTime> queryMethodResponseTimes(Pageable pageable, String urlId,String applicationId) {
		Date startDate = LocalDate.now().toDate();
		Date endDate = LocalDateTime.now().toDate();
		...
		for(Method method : methods) {
			MethodResponseTime methodResponseTime = methodResponseTimeRepository.selectMethodResponseTimes(urlId, method.getId(), startDate, endDate,applicationId);
            methodResponseTime = (methodResponseTime==null)?new MethodResponseTime():methodResponseTime;
            ...
            methodResponseTime.initViewValues();
